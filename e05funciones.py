from e00funciones import imprimir_dato
from e04funciones import stark_normalizar_datos
import re
import json
import copy
...
def imprimir_menu_desafio():
    """
    No recibe nada
    imprime un menu de opciones numerico por pantalla y 0 para salir
    Utiliza la funcion imprimir_dato    
    """
    menu = "Menu numerico:\nOpcion 1.\nOpcion 2.\nOpcion 3.\nOpcion 4.\nOpcion 5.\nOpcion 6.\n0: Salir"
    imprimir_dato(menu)

def menu_principal_desafio():
    imprimir_menu_desafio()
    opcion = input("Elija una opcion: ").upper()
    if re.search("[0-6]", opcion):
        return opcion
    else:
        print("Opcion equivocada")
    
def app_desafio(lista:list):
    stark_normalizar_datos(lista)
    while True:
        opcion = menu_principal_desafio()
        if opcion == "0":
            break
        elif opcion == "1":
            print("1")
...
def stark_marvel_app_5(lista:list):
    ...
    while True:
        ...
        if opcion == "A":
            ...
            print("Opcion erronea")    
#1.4
def leer_archivo(archivo_extension:str):
    """
    Recibe como parametro un string que indica el nombre del archivo y su extension(archivo.json)
    y se abre el archivo en modo lectura y retorna la lista como una lista de dicts
    """
    with open(archivo_extension, "r") as file:
        data = json.load(file)
    return data

def leer_archivo_csv(archivo_extension:str):
    with open(archivo_extension, "r") as file:
        data = file.read()#readlines devuelve lista (read string)
        data_copia = copy.copy(data)
    return data_copia
...
def capitalizar_palabras(palabras:str):
    """
    Recibe un string con una o muchas palabras
    Y Retorna un string en el cual todas y cada una de las palabras son capitalizadas
    """
